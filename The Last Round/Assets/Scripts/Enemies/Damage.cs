...
public class Damage : MonoBehaviour
{
    ...
    private void Hurt(GameObject collision)
    {
            ...
            ((!ImEnemy && IsEnemy) || (ImEnemy && !IsEnemy)))
        {
            ...
            {
                mejoraDmg = GameManager.Instance.GetMeleeDamagePercent() * Basedamage * GameManager.Instance.GetUpgradeLevel(1); //mejora un 10% el daño por cada nivel de mejora
            }
            else if (GetComponent<BulletMovement>() != null) //si es la bala
            {
                mejoraDmg = GameManager.Instance.GetRangeDamagePercent() * Basedamage * GameManager.Instance.GetUpgradeLevel(0);
            }
